...
%%split
 ...
 if ((Tx_ub-Tx_lb)>=(Ty_ub-Ty_lb))%split x
     ...
     Tx_opt1=child1_lp(1);
     Ty_opt1=child1_lp(2);
     inlier_lb1=sqrt(sum(sum((abs(delta_x+Tx_opt1)<delta).* ((abs(delta_y+Ty_opt1)<delta))')));
     ...
     Tx_opt2=child2_lp(1);
     Ty_opt2=child2_lp(2);
     inlier_lb2=sqrt(sum(sum((abs(delta_x+Tx_opt2)<delta).* ((abs(delta_y+Ty_opt2)<delta))')));
